            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        github = GitHub(TokenAuthStrategy(token))
                        return
                    logger.error(
...
async def fetch_github_data(repo: str, endpoint: str) -> dict | list | None:
    """
    Fetch data from the GitHub API
    for a specific repo and endpoint.
    """

    global api_cache, github
    logger.info(f"GitHub auth strategy: {type(github.auth)}")
    ...
    async def get_data(github: GitHub, endpoint: str, owner: str, repository: str) -> dict | list:
        match endpoint:
            case "commits":
                got_repo: Response[list[Commit]] = await github.rest.repos.async_list_commits(repo=repository,owner=owner)
            case "issues":
                got_repo: Response[list[Issue]] = await github.rest.issues.async_list_for_repo(repo=repository,owner=owner)
            case "pulls":
                got_repo: Response[list[PullRequest]] = await github.rest.pulls.async_list(repo=repository,owner=owner)
            case "releases":
                got_repo: Response[list[Release]] = await github.rest.repos.async_get_latest_release(repo=repository,owner=owner)
            case _:
        ...
        return got_repo.json()


    try:
        data = await get_data(github, endpoint, owner, repository)
    except RetryError as e:
        if not "404 Not Found" in e.last_attempt.exception().args:
            logger.error(f"Failed to fetch {endpoint} for {repo} after {max_retries} attempts: {e.last_attempt.exception()}")
            return None
...
async def notify(
    ...
    latest_data: list = data[:5] if isinstance(data,list) else [data]
