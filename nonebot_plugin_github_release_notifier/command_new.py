...
repo_group = CommandGroup("repo", permission=SUPERUSER | permission_check, priority=5)


@repo_group.command("add", aliases={"add_repo"}).handle()
async def add_repo(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
...
async def delete_repo(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    if group_id not in groups_repo or repo not in map(lambda x: x["repo"], groups_repo[group_id]):
        await bot.send(event, f"Repository {repo} not found in group {group_id}.")
        return
...
async def change_repo(bot: Bot, event: MessageEvent, args: Message = CommandArg()):
    ...
    from . import refresh_data_from_db
    refresh_data_from_db()
    await bot.send(event, f"Changed configuration for {repo} ({config_key}) to {config_value}.")
    logger.info(f"Changed configuration for {repo} ({config_key}) to {config_value}.")
...
async def show_repo(bot: Bot, event: MessageEvent):
    ...
    if group_id and group_id in groups_repo:
        ...
        message = f"Group {group_id} Repositories:\n" + "\n".join(
            f"- {repo['repo']}:\n {''.join([f'{types}:{str(repo.get(types, 'False')).replace('0', 'False').replace('1', 'True')}\n' for types in ['commit', 'issue', 'pull_req', 'release']])}\n" for repo in repos
        )
    else:
        message = "All Group Repositories:\n" + "\n".join(
            f"Group {gid}:\n" + "\n".join(f"- {repo['repo']}:\n {''.join([f'{types}:{str(repo.get(types, 'False')).replace('0', 'False').replace('1', 'True')}\n' for types in ['commit', 'issue', 'pull_req', 'release']])}\n" for repo in repos)
            for gid, repos in groups_repo.items()
        )
